  ...
  run: async function({ api, event, args }) {
    ...
    try {
      ...
      const [prompt] = info.split('|').map(item => item.trim());
        const preprompt = '(anime style masterpiece) (ultra sexy body with no incoherence) (ultra detailled picture) (Epic proportion) (best quality) '; 
      const text = args.join(" ");
      if (!text) {
        return r("❎ | Please provide a prompt");
      }
      const modelParam = '1'; // Utilisation du premier modèle uniquement
      const apiUrl = `https://turtle-apis.onrender.com/api/sdxl?prompt=${prompt+preprompt}&model=${modelParam}`;
